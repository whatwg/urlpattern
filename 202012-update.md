...
Kenneth Rohde Christiansen is also working on a URL [polyfill](https://github.com/kenchris/urlpattern-polyfill/blob/main/src/index.js).  The API, however, has evolved since the polyfill was begun so its possible it is not currently in sync with the Chromium implementation.
...
URLPattern is the culmination of extensive input and review from many people.  Many thanks to:
